...
@router.get('/')
async def main_get(request: Request, db: AsyncSession = Depends(get_db)):
    pages = []
    if request.session.get('username'):
        pages = await get_all_hosts(db, request.session.get('username'))
...
async def create_post(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    await form.is_valid()
    if not form.errors:
        if await create_host(db, form, request.session.get('username')):
...
async def site(request: Request, url: str, db: AsyncSession = Depends(get_db)):
...
async def delete(request: Request, url: str, db: AsyncSession = Depends(get_db)):
    host = await get_host(db, url)
    ...
    await delete_host(db, url)
    return RedirectResponse(request.url_for('main_get'), status_code=303)


@router.get('/update/{url:path}')
async def update_get(request: Request, url: str, db: AsyncSession = Depends(get_db)):
    host = await get_host(db, url)
    if host.user.username != request.session.get('username'):
...
async def update_post(request: Request, url: str, db: AsyncSession = Depends(get_db)):
    host = await get_host(db, url)
    if host.user.username != request.session.get('username'):
        return RedirectResponse(request.url_for('main_get'), status_code=303)
    form = HostForm(request)
    ...
    await form.is_valid()
    if not form.errors:
        if await update_host(db, form, url):
            return RedirectResponse(request.url_for('main_get'), status_code=303)
